...
- Every service you turn on translates at once, each in its own block you can collapse:
  your agent, macOS's own translation, Google Translate, Microsoft Translator and DeepL.
  Turn services on or off on the Translate page in Settings.
  - Your agent needs nothing set up, and it is the only service that can also adjust a
    translation when you tell it what to change.
  - macOS translation works on-device and needs the language pair downloaded first
...
- 剪贴板条目和截图卡片上有同一个按钮。截图先用设备上的 `ocr` 读出文字，送出去的只有
  文字，不是图片。
- 你在设置里打开的每个服务都会同时翻译，各自成块、可以收起：你的助手、macOS 自带翻译、
  Google 翻译、微软翻译和 DeepL。开关都在设置的「翻译」页里。
  - 你的助手不用配置，而且只有它能在译完后接着按你的要求调整译文。
  - macOS 自带翻译在设备上完成，需要先下载对应的语言（系统设置 › 通用 › 语言与地区）。
